...
public class ProductsDAO {
    ...
    /**
     * export method(): used to export all inventory data into a CSV file.
     * @return an array list containing array lists that stores data about a products attributes.
     * @throws SQLException
     */
    public ArrayList<ArrayList<Object>> exportCSV() throws SQLException {
        String products = "SELECT * FROM Products";
        Connection connection = null;
        PreparedStatement selectStmt = null;
        ResultSet result = null;
        try {
            ...
            while (result.next()) {
                ...
                double resultPrice = result.getDouble("price");
                String resultCurrency = result.getString("currency");
                productList.add(resultID);
                productList.add(resultName);
                productList.add(resultQuantity);
                productList.add(resultPrice);
                productList.add(resultCurrency);
                output.add(productList);
            }
            return output;
        } catch (SQLException e) {
            e.printStackTrace();
